...
st.caption('The camera cannot work on servers without SSL sertifications..')

input_type = st.radio(
    "Select the image input method",
    ["Upload image", "Webcam"]
)
...
if bg_image:
    ...
    if st.button('Fill holes'):
        if not bg_image:
            ...
            st.header('Summary of PSNR, MSE, SSIM values')
